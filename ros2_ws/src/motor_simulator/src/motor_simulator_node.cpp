...
public:
    MotorSimulatorNode()
        ...
        RCLCPP_INFO(get_logger(), "电机模拟器启动");
    }
...
private:
    void data_publish() {
        ...
        double velocity = motor_model_.get_velocity();
        ...
        velocity_msg.set__data(velocity);
        ...
        if (debug_count++ % 200 == 0) {
            RCLCPP_INFO(get_logger(), "基础速度: %.1f", velocity);
        }
    }
    ...
    MotorBaseModel motor_model_{5e-4, 2e-3, 3.0, 0.0001};
